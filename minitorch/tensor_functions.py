...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        # TODO: Implement for Task 2.4.
        (t1,) = ctx.saved_tensors
        tmp = grad_output.f.add_zip(tensor([1.0]), grad_output.f.neg_map(t1)) # 1 - sig
        tmp = grad_output.f.mul_zip(tmp, t1) # sig * (1 - sig)
        return grad_output.f.mul_zip(tmp, grad_output)
